...
export function identify(state, keyHash, masterKey) {
  ...
  const m = members.find(x => (x.pw && x.pw === keyHash) || (x.pwNew && x.pwNew === keyHash));
  if (!m) return null;
...
export function filterForUser(state, user) {
  ...
  const out = { ...state };        // flach kopieren, nicht den ganzen Baum

  // Passwörter verlassen den Server nie
  out.members = (state.members || []).map(m => {
    const { pw, pwNew, ...rest } = m;
    return { ...rest, hasPw: !!(pw || pwNew) };
  });

  const seen = visibleSet(state, user);
  out.groups = (state.groups || []).map(g => ({
    ...g,
...
export async function handle(req, store, cfg) {
  ...
  if (keyHash && keyHash === master) {
    ...
    try {
      const mem = store.loadMembersRaw ? JSON.parse((await store.loadMembersRaw()) || "null") : null;
      const list = mem || JSON.parse((await store.loadStateRaw()) || "null")?.members || [];
      const m = list.find(x => (x.pw && x.pw === keyHash) || (x.pwNew && x.pwNew === keyHash));
  ...
  if (url.pathname.endsWith("/file")) {
    if (req.method === "GET") {
      ...
      // Rohtext statt JSON: spart Rechenzeit bei großen Dateien
      return new Response(f.data, { status: 200, headers: {
        "content-type": "text/plain; charset=utf-8", "cache-control": "no-store",
        "x-file-name": encodeURIComponent(f.name || ""), "x-file-type": f.type || "" } });
    }
    if (req.method === "POST") {
      const p = url.searchParams;
      let id, name, type, task, ts, data;
      if (p.get("id")) {                       // Daten pur im Rumpf, Angaben in der Adresse
        id = p.get("id");
        name = decodeURIComponent(p.get("name") || "datei");
        type = p.get("type") || "";
        task = p.get("task") || "";
        ts = Number(p.get("ts")) || Date.now();
        data = await req.text();
      } else {                                 // ältere Form
        let b; try { b = await req.json(); } catch { return json({ error: "bad json" }, 400); }
        if (!b) return json({ error: "no file" }, 400);
        ({ id, name, type, task } = b); ts = Number(b.ts) || Date.now(); data = b.data;
      }
      if (!id || !data) return json({ error: "no file" }, 400);
      if (String(data).length > 12_000_000) return json({ error: "file too large" }, 413);
      await store.putFile({ id: String(id), name: String(name || "datei"), type: String(type || ""),
                            ts, task: String(task || ""), data: String(data) });
  ...
  /* ---- Stand speichern ----
     Der Browser schickt den Stand pur und die Revision in der Adresse.
     Für den Inhaber wird ein großer Stand unverändert abgelegt, ohne ihn
     zu zerlegen – sonst scheitert das Speichern am Rechenlimit. */
  if (req.method === "PUT") {
    const text = await req.text();
    const qRev = url.searchParams.get("rev");
    let body = null, stateRaw = null, clientRev = null;

    if (qRev !== null) {                      // neue Form: nur der Stand im Rumpf
      stateRaw = text;
      clientRev = Number(qRev);
    } else {                                  // alte Form: { rev, state }
      try { body = JSON.parse(text); } catch { return json({ error: "bad json" }, 400); }
      if (!body || typeof body.state !== "object" || body.state === null) {
        return json({ error: "no state" }, 400);
      }
      stateRaw = JSON.stringify(body.state);
      clientRev = typeof body.rev === "number" ? body.rev : null;
    }

    if (clientRev !== null && clientRev !== rev && rev > 0) {
      const stateRaw = await store.loadStateRaw();
      if (stateRaw) {
    ...
    const GROSS = 400_000;          // ab dieser Größe wird nicht mehr zerlegt
    const nextRev = rev + 1;
    const mailMeta = (meta && meta.mail) || { sent: {} };
    const nowTs = Date.now();

    /* Sonderweg: sehr großer Stand vom Inhaber – unverändert ablegen.
       Kein Zerlegen, kein Zusammenbauen, damit das Rechenlimit hält.
       Die Passwörter sind darin bereits enthalten, weil der Inhaber den
       Stand unverändert erhalten hat. */
    if (user.admin && stateRaw.length > GROSS) {
      await store.saveState(nextRev, stateRaw, mailMeta);
      return json({ rev: nextRev, big: true });
    }

    let nextState;
    if (user.admin) {
      nextState = JSON.parse(stateRaw);
      try {                                   // Passwörter bewahren
      ...
      nextState = applyIncoming(stored, JSON.parse(stateRaw), user);
    }
